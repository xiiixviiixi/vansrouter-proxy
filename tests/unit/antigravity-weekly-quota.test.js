...
describe("parseWeeklyQuotaSummary", () => {
  ...
  it("handles alternate nested quotaSummary.groups shape", () => {
    const result = parseWeeklyQuotaSummary({ quotaSummary: { groups: [GEMINI_GROUP, CLAUDE_GPT_GROUP] } });
  ...
  it("returns empty object for malformed input", () => {
    ...
    expect(parseWeeklyQuotaSummary({ groups: [{ displayName: "Gemini Models" }] })).toEqual({});
  });
  ...
  it("handles bucket with non-finite remainingFraction", () => {
    const data = {
      groups: [{
        displayName: "Gemini Models",
        buckets: [{ bucketId: "weekly-bucket", displayName: "Weekly", remainingFraction: "not-a-number" }],
      }],
    };
    expect(parseWeeklyQuotaSummary(data)).toEqual({});
  });
  ...
  it("ignores groups that don't match known families", () => {
    const data = {
      groups: [{
        ...
        buckets: [{ bucketId: "weekly-bucket", displayName: "Weekly", remainingFraction: 0.5 }],
      }],
    };
    expect(parseWeeklyQuotaSummary(data)).toEqual({});
  });
...
describe("fetchAntigravityWeeklyQuota", () => {
  ...
  it("fetches and returns parsed weekly quota on success", async () => {
    proxyAwareFetch.mockResolvedValue({ ok: true, json: async () => FULL_RESPONSE });
  ...
  it.each([401, 403, 404, 429])("returns {} on HTTP %i", async (status) => {
    proxyAwareFetch.mockResolvedValue({ ok: false, status });
    expect(await fetchAntigravityWeeklyQuota("token", "project-1")).toEqual({});
  });
  ...
  it("deduplicates concurrent requests for the same account", async () => {
    let resolveResponse;
    proxyAwareFetch.mockReturnValue(new Promise((resolve) => { resolveResponse = resolve; }));
  ...
  it("sends correct headers and body", async () => {
    proxyAwareFetch.mockResolvedValue({ ok: true, json: async () => ({ groups: [] }) });
...
// — Integration with getAntigravityUsage ——————————————————
describe("weekly quota in getAntigravityUsage", () => {
  beforeEach(() => {
    proxyAwareFetch.mockReset();
    _clearWeeklyCache();
  });

  function mockCalls({ paidTier, models, summary, summaryThrows = false }) {
    proxyAwareFetch.mockImplementation(async (url) => {
      if (url.includes(":loadCodeAssist")) {
        return {
          ok: true,
          status: 200,
          json: async () => ({ cloudaicompanionProject: "p1", currentTier: { name: "Pro" }, ...(paidTier ? { paidTier } : {}) }),
        };
      }
      ...
      if (url.includes(":retrieveUserQuotaSummary")) {
        if (summaryThrows) throw new Error("weekly endpoint unavailable");
  ...
  const PAID = { id: "g1-pro-tier", name: "Google AI Pro" };

  it("overlays weekly rows on paid-tier per-model rows without dropping them", async () => {
    mockCalls({
      paidTier: PAID,
      models: {
    ...
    expect(result.quotas.gemini_weekly).toMatchObject({ remainingPercentage: 75 });
    expect(result.quotas.claude_gpt_weekly).toMatchObject({ remainingPercentage: 50 });
  });

  it("keeps per-model quota results when the weekly endpoint fails", async () => {
    mockCalls({
      paidTier: PAID,
      models: {
        "gemini-3.8-flash-high": {
    ...
    expect(result.quotas["gemini-3.8-flash-high"]).toMatchObject({ remainingPercentage: 85 });
  ...
  it("free-tier accounts only show weekly quotas, not per-model short-window quotas", async () => {
    mockCalls({
      ...
      summary: {
        groups: [
          {
            displayName: "Gemini Models",
            buckets: [{ bucketId: "gemini-weekly", displayName: "Weekly Limit Remaining", remainingFraction: 1, resetTime: "2026-09-15T00:00:00Z" }],
          },
          {
            displayName: "Claude and GPT models",
            buckets: [{ bucketId: "3p-weekly", displayName: "Weekly Limit Remaining", remainingFraction: 0, resetTime: "2026-09-13T12:00:00Z" }],
          },
        ],
      },
    });
    ...
    expect(result.quotas.gemini_weekly).toMatchObject({ used: 0, total: 1000, remainingPercentage: 100 });
    expect(result.quotas.claude_gpt_weekly).toMatchObject({ used: 1000, total: 1000, remainingPercentage: 0 });
  });

  it("trusts the reported weekly quota when family models are exhausted", async () => {
    mockCalls({
      paidTier: PAID,
      models: {
        "gemini-3.8-flash-high": {
    ...
    // The 5h window is spent, not the weekly one — the summary stays authoritative
