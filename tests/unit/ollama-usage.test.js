...
describe("parseQuotaData(ollama)", () => {
  // The fork routes ollama through parseQuotaData's generic fallback (no dedicated
  // case), which drops remainingPercentage but keeps used/total/resetAt — QuotaTable
  // derives its bar from getRemainingPercentage(), so the rendered result is the same.
  it("keeps the quota rows and their reset in the dashboard shape", () => {
    const rows = parseQuotaData("ollama", {
      ...
      quotas: {
        "Monthly": {
          ...
          total: 100,
          remainingPercentage: 98,
          resetAt: "2026-10-06T22:15:39.000Z",
        },
      },
    });
    ...
    expect(getRemainingPercentage(rows[0])).toBe(98);
  });
});
