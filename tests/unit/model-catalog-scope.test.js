...
describe("model catalog", () => {
  ...
  it("passes the gateway to the catalog reader when refining", () => {
    ...
    try {
      // the *deepseek* pattern resolves this id, so refine() runs
      expect(capabilities.getCapabilitiesForModel("gateway-a", "deepseek-v9-probe").vision).toBe(true);
      expect(capabilities.getCapabilitiesForModel("gateway-b", "deepseek-v9-probe").vision).toBe(false);
  ...
  it("shares the installed source with every copy of the module", async () => {
    ...
    try {
      ...
      expect(other.getCapabilitiesForModel("gateway-a", "deepseek-v9-probe").vision).toBe(true);
    } finally {
      capabilities.setCatalogSource(null);
    }
