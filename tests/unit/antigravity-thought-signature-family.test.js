...
const { DEFAULT_THINKING_GEMINI_CLI_SIGNATURE, DEFAULT_THINKING_AG_SIGNATURE } = await import("../../open-sse/config/defaultThinkingSignature.js");
...
describe("antigravity executor backfills from the store", () => {
  // Envelope the translator hands to the executor: history lost its signatures.
  function executorBody(callId) {
    return {
      request: {
        contents: [
          { role: "user", parts: [{ text: "list files" }] },
          { role: "model", parts: [{ functionCall: { id: callId, name: "ls", args: {} } }] },
        ],
      },
    };
  }

  function sendSignature(model, callId) {
    const out = new AntigravityExecutor().transformRequest(model, executorBody(callId), true, { projectId: "p", connectionId: "c" });
    return out.request.contents.flatMap((c) => c.parts).find((p) => p.functionCall)?.thoughtSignature;
  }

  it("replays a cached signature of the same family instead of the default", () => {
    const call = uid("call");
    storeGeminiThoughtSignature(call, "GEMINI_SIG", null, "gemini-3.8-flash-tiered");
    expect(sendSignature("gemini-3.8-flash-tiered", call)).toBe("GEMINI_SIG");
  });

  it("ignores a cached signature from the other family and sends the default", () => {
    const call = uid("toolu");
    storeGeminiThoughtSignature(call, "CLAUDE_SIG", null, "claude-opus-4-6-thinking");
    expect(sendSignature("gemini-3.8-flash-tiered", call)).toBe(DEFAULT_THINKING_AG_SIGNATURE);
  });
});
