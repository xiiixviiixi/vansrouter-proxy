...
//
// The router is a switchboard in front of many inference providers, addressed as
// `<baseUrl>/<provider>/<providerModelId>`. `providerModelId` is the id the *provider*
// uses — not the Hub model id — so it is resolved through `imageConfig.modelMap`.
//
// The legacy `api-inference.huggingface.co` host is retired (DNS ENOTFOUND) and must not
// be referenced here.
...
// A connection may point at its own endpoint (self-hosted TGI container). That endpoint
// knows its own model ids, so the router mapping does not apply and the Hub id passes through.
...
export default {
  buildUrl: (model, creds) => {
    ...
    if (override) {
      // The model id is client-controlled; on a custom endpoint it lands in a URL path
      // verbatim, so reject traversal/query injection (mirrors sttCore's guard).
